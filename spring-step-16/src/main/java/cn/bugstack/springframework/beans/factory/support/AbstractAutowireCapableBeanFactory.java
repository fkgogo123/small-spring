...
//         判断是否返回代理bean对象，转移到了初始化过程的后置处理中。
        Object bean = resolveBeforeInstantiation(beanName, beanDefinition); // 这一步骤实际上没有进行任何操作。
        if (null != bean) {
            return bean;
        }
...
//            bean = beanDefinition.getBeanClass().newInstance();
            ...
            // 处理循环依赖，将实例化后的 bean 对象提前存储到缓存中
            if (beanDefinition.isSingleton()) {
                Object finalBean = bean;
                // 通过这个方法 【提前暴露】 bean，在三级缓存中放置一个 工厂对象。
        ...
        if (beanDefinition.isSingleton()) {
            // 在从三级缓存中的工厂对象获取对象时，会调用getObject方法，会判断是否需要代理，从而获取目标对象 or 代理对象。
            // bug：三级缓存中获取的代理对象会将初始化过程中获取的代理对象覆盖掉。
            // 但是二者代理的目标对象是同一个，finalBean是一个指针。
